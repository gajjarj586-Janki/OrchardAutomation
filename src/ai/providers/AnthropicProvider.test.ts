...
function makeConfig(model = 'claude-test-model'): FrameworkConfig {
...
/** A fake Anthropic client whose `messages.create` returns one canned text reply. */
function fakeClient(text: string): Pick<Anthropic, 'messages'> {
  return {
    messages: {
      create: (async () => ({
...
test('constructor throws a FrameworkError when AI_API_KEY is missing and no client is injected', () => {
  ...
  try {
    assert.throws(() => new AnthropicProvider(makeConfig()), /AI_API_KEY/);
  } finally {
...
test('constructor throws a FrameworkError when AI_MODEL is empty', () => {
  assert.throws(
    () => new AnthropicProvider(makeConfig(''), fakeClient('irrelevant')),
    /AI_MODEL/
  );
});
...
test('generateFeature produces valid, actionable Gherkin', async () => {
  ...
  const provider = new AnthropicProvider(makeConfig(), fakeClient(gherkin));
...
test('suggestLocator produces output matching AISuggestedLocatorListSchema', async () => {
  ...
  const provider = new AnthropicProvider(makeConfig(), fakeClient(json));
...
test('suggestLocator throws a clear FrameworkError when the model does not return JSON', async () => {
  const provider = new AnthropicProvider(makeConfig(), fakeClient('not json at all'));
...
test('analyzeFailure falls back to the deterministic summary when the AI call fails', async () => {
  const throwingClient: Pick<Anthropic, 'messages'> = {
    messages: {
      create: (async () => {
  ...
  const provider = new AnthropicProvider(makeConfig(), throwingClient);
